...
//    Created: 07/01/2015 | Developer: reyro | Description: Save (create and update)
    public function save() {
        if (isset($this->passedArgs['id'])) {//UPDATE
            $this->AdmModule->id = $this->passedArgs['id'];
            if (!$this->AdmModule->exists()) {
                $this->redirect(array('action' => 'index'));
            }
            $this->AdmModule->recursive = -1;
            $this->request->data = $this->AdmModule->read(null, $this->passedArgs['id']);
        }
    }
...
//    Created: 07/01/2015 | Developer: reyro | Description: Function save (create and update) | Request: Ajax
    public function fnSave() {
        if ($this->RequestHandler->isAjax()) {
            ...
            $data['AdmModule']['initials'] = strtolower($data['AdmModule']['initials']);
            try {
                if ($this->AdmModule->save($data)) {
                    $response = $this->BittionMain->fnGetMethodResponse('SUCCESS', array('data' => array('id' => $this->AdmModule->id)));
                }
            } catch (Exception $exc) {
