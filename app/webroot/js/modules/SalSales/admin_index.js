...
//    Created: 25/09/2014 | Developer: reyro | Description: initiate DataTable
    var table1 = jQuery('#SalSaleIndexDT').DataTable({
        /////////Ajax
        'bServerSide': true
        , 'sAjaxSource': bittionUrlProjectAndController + 'fnAdminRead'
        , 'sServerMethod': 'POST'
        ...
        , 'fnCreatedRow': function (nRow, aData, iDataIndex) {
            ...
            var cssInvoiceWrap = fnPutCssWrapIfInvoice(aData[6]['control_code']);
            jQuery('td:eq(0)', nRow).html(aData[0]+' '+cssInvoiceWrap);
...
//    Created: 06/06/2015 | Developer: reyro | Description: Put css wrap if it's invoice
    function fnPutCssWrapIfInvoice(controlCode) {
        if(controlCode){
...
//    Created: 25/09/2014 | Developer: reyro | Description: generates Html Row Buttons Edit and Delete
    function fnGenerateHtmlTableButtons(data) {
        var buttonHtml = '<a href="' + bittionUrlProjectAndController + 'admin_save/id:' + data['id'] + '" class="btn btn-sm btn-primary" title="Editar"><i class="fa fa-pencil"></i> </a>';
